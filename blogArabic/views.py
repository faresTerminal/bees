...
from blogArabic.forms import createForm, createAuthor, CommentForm, ImageForm
from django.forms import ModelForm
from django.contrib.auth.models import User
from blogArabic.models import author, articles, Category, contactModel, comment_put, MyltiImage
...
def index(request):
           ...
           return render(request, 'home/index.html', {'all_articles': all_articles, 'cate':all_category, 'solo1':solo1, 'full_name': request.user.username})
...
def create_article(request):
    ...
    ImageFormSet = modelformset_factory(MyltiImage,
    ...
    if request.user.is_authenticated:
        ...
        if author_profile:
          ...
          return render(request, 'home/post.html', {"form": form, 'formset': formset, 'full_name': request.user.username})
...
def publish(request):
        ...
        if request.user.is_authenticated:
          ...
          formset = ImageFormSet(request.POST or None, request.FILES or None, queryset=MyltiImage.objects.none())
        ...
        #pass from data base to template
            ...
            redirect_to = reverse('blogArabic:show_article', kwargs={'id': instance.id})
            return redirect(redirect_to)
          return render(request, 'home/index.html', {'form': form, 'formset': formset, 'user': authorUser, 'full_name': request.user.username})
...
def show_article(request, id):
    ...
    three = articles.objects.order_by('-id')[:8]
    photo =  MyltiImage.objects.all().filter(post = id)
    # when we have 10 post with this less 2 article[::-2]
...
def like_post(request):
   ...
   #post = get_object_or_404(articles, id=request.POST.get('id'))
    ...
    redirect_to = reverse('blogArabic:show_article', kwargs={'id': post.id})
